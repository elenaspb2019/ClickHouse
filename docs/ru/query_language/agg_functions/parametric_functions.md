...
- `window` — ширина скользящего окна по времени в секундах. [UInt](../../data_types/int_uint/).
...
- `cond` — условия или данные, описывающие цепочку событий.[UInt8](../../data_types/int_uint/#diapazony-uint) Значения могут быть 0 или 1.
...
Чтобы узнать, как далеко пользователь `user_id` смог пройти по цепочке за период в январе-феврале 2017-го года, составим запрос:
