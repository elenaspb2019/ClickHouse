...
Аналитическая функция, которая показывает, насколько продукт или сервис способен удержать клиента втечение заданного периода времени.
Функция принимает в качестве аргументов набор из выражений (от 1 до 32) и применяет их к заданному набору данных.
В качестве аргумента указывается любое логическое условие (как в [WHERE](https://clickhouse.yandex/docs/ru/query_language/select/#select-where)).
Условия, за исключением первого, применяются попарно: результат второго будет истинным, если истинно первое и второе, третьего - если истинно первое и третье и т. д.
...
- `cond` — вычисляемое условие или выражение.

**Возвращаемое значение**

Функция возвращает набор массивов, где количество массивов соответсвует набору данных, а длина каждого - количеству условий, переданных в функцию. 
Значение каждого элемента - `0` или `1` в зависимости от результата вычисления условия.

Тип: `UInt8`.

**Пример**

Приведем пример расчета удержания клиентов для аналитики сайта.
...
└────────────┴─────┘
```
Используем функцию `retention` в запросе и сгруппируем пользователей по уникальному идентификатору `uid`.
...
`r1` - количество уникальных посетителей, удовлетворяющих условию `cond1`. 

`r2` - количество уникальных посетителей, удовлетворяющих условиям `cond1` и `cond2`. 

`r3` - количество уникальных посетителей, удовлетворяющих условиям `cond1` и `cond3`.
...
```
Количество посещений сайта пользователями за день.
