...
Вычисляет [медиану](https://ru.wikipedia.org/wiki/Медиана_(статистика)) для числовой выборки данных.

Функция использует метод [reservoir sampling](https://ru.wikipedia.org/wiki/Reservoir_sampling) с размером резервуара до 8192. Этот алгоритм основан на использовании генератора случайных чисел, следовательно функция является недетерминированной. Имеет низкую точность. Для более точного реузультата используйте фукцию [medianExact](#medianexact).
...
Функция является алиасом для [quantile(0.5)(x)](#agg_function-quantile).
...
- Среднее по порядку значение для выборки с нечетным числом значений.
- Среднее арифметическое двух средних значений для набора с четным числом значений. Функция не округляет результат.  
...
Вычисляет медиану числового набора данных. Функция использует метод [reservoir sampling](https://en.wikipedia.org/wiki/Reservoir_sampling) с размером резервуара до 8192. 
Этот алгоритм обеспечивает низкую точность.
...
Функция является алиасом для [quantileDeterministiс(0.5)(x)](#agg_function-quantiledeterministic).
...
- `determinator` - Число, хэш которого используется вместо генератора случайных чисел в алгоритме [reservoir sampling](https://en.wikipedia.org/wiki/Reservoir_sampling). Чтобы функция работала правильно, одно и то же значение детерминатора не должно встречаться слишком часто. В качестве детерминатора можно использовать идентификатор события, идентификатор посетителя и т. п. Используется целое число (0 или больше).
...
Функция точно вычисляет медиану. В алгоритме используется частично отсортированный массив.
Функция потребляет O(n) памяти, где 'n' - количество значений.
...
Функция является алиасом для [quantileExact(0.5)(x)](#quantileexact).
...
- Среднее по порядку значение для выборки с нечетным числом значений.
- Для набора с четным числом значений будет возвращено большее из двух средних значений.
...
Вычисляет медиану точно в соответствии с весом каждого значения. Вес означает, что значение встречается `weight` раз.
Сама функция может рассматриваться как суммирование гистограмм.

Эта функция работает более эффективно, чем [median Exact](#medianexact), поскольку она использует хэш-таблицу в алгоритме.
...
Функция является алисом для [quantileExactWeighted(0.5)(x)](#quantileexactweighted).
...
- Среднее по порядку значение для выборки с нечетным числом значений.
- Для набора с четным числом значений будет возвращено меньшее из двух средних значений.
...
Вычисляет медиану с определенной точностью. Оптимизирована для обработки временных меток unix.
Результат является детерминированным. Функция предназначена для анализа временных данных, таких как время загрузки страницы.
...
Функция является алиасом для [quantileTiming(0.5)(x)](#agg_function-quantiletiming).
...
Оптимизирована для обработки временных меток unix в соответствии с весом каждого значения. Вес означает, что значение встречается `weight` раз.
Вычисляет медиану с определенной точностью.
Результат детерминирован. Функция предназначена для анализа временных данных, таких как время загрузки страницы.
...
Функция является алиасом для [quantileTimingWeighted(0.5)(x)](#quantiletimingweighted).
...
Вычисляет медиану числового набора данных с помощью алгоритма [T-digest](https://github.com/tdunning/t-digest/blob/master/docs/t-digest-paper/histo.pdf).
Вычисляемое значение зависит от порядка выполнения запроса и не является детерминированным.

Максимальная ошибка составляет 1%, а потребление памяти log(n), где 'n' - количество значений.
...
Вычисляет медиану числового набора данных с помощью алгоритма [T-digest](https://github.com/tdunning/t-digest/blob/master/docs/t-digest-paper/histo.pdf) в соответствии с весом каждого значения. Вес означает, что значение повторяется `weight` раз.
Результат зависит от порядка выполнения запроса и не является детерминированным.
